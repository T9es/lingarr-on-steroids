...
public class TranslationCompareController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            request.Id, cancellationToken);
                ...
                var hydration = await FailedTranslationCheckpointRules.HydrateAsync(
                    request,
                    ...
                    cancellationToken,
                    request.JobId);
                missingPositions = FailedTranslationCheckpointRules.GetMissingPositions(
                    originalSubtitles,
                    ParseMissingPositions(request.Id),
                    hydration);
            ...
            var response = new CompletedTranslationCompareResponse
            {
                ...
                SourceFingerprint = await ComputeCurrentSourceFingerprintAsync(
                    request,
                    originalSubtitle.Path,
                    cancellationToken),
        ...
        finally
        {
            ...
            CleanupTemporaryTranslatedSubtitle();
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var sourceFingerprint = await ComputeCurrentSourceFingerprintAsync(
                request,
                originalSubtitle.Path,
                cancellationToken);

            if (string.IsNullOrWhiteSpace(editRequest?.SourceFingerprint))
            {
                return Conflict(new
                {
                    code = "source-fingerprint-required",
                    message = "A current source fingerprint is required. Reload the compare view before saving or accepting edits."
                });
            }

            if (!string.Equals(
                    editRequest.SourceFingerprint,
                    sourceFingerprint,
                    StringComparison.Ordinal))
            {
                return Conflict(new
                {
                    ...
                    message = "The source subtitle changed after the compare view was loaded. Reload the compare view before saving or accepting edits."
                });
            }
            ...
            var hydration = await FailedTranslationCheckpointRules.HydrateAsync(
                ...
                cancellationToken,
                request.JobId);
            if (!TryBuildCanonicalEdits(
                    editRequest?.Edits ?? [],
                    originalSubtitles,
                    originalSubtitle.Path,
                    out var edits,
                    out var editConflictMessage))
            {
                return BadRequest(new { message = editConflictMessage });
            }
            var completion = await _failedCompletionService.CompleteAsync(
                request,
                edits,
                hydration.SourceTextPositions.ToHashSet(),
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        ResolvedSubtitlePath? originalSubtitle = null;
        string? failedSaveOwnershipToken = null;
        ...
        try
        {
            ...
            if (request.Status == TranslationStatus.Failed)
            {
                failedSaveOwnershipToken = $"failed-compare-save-{Guid.NewGuid():N}";
                var claimRowsUpdated = await _dbContext.TranslationRequests
                    .Where(item => item.Id == request.Id &&
                                   item.Status == TranslationStatus.Failed &&
                                   item.JobId == null)
                    .ExecuteUpdateAsync(setters => setters
                        .SetProperty(item => item.JobId, failedSaveOwnershipToken)
                ...
                if (claimRowsUpdated == 0)
                {
                    return Conflict(new
                    {
                        code = "request-state-changed",
                        message = "The failed translation request changed while saving edits. Reload the compare view and try again."
                    });
                }
                ...
                hydration = await FailedTranslationCheckpointRules.HydrateAsync(
                    request,
                    originalSubtitle.Path,
                    checkpoint,
                    originalSubtitles,
                    ...
                    cancellationToken,
                    failedSaveOwnershipToken);
                var missingPositions = FailedTranslationCheckpointRules.GetMissingPositions(
                    originalSubtitles,
                    ParseMissingPositions(request.Id),
                    hydration);
            ...
            var publication = await _failedCompletionService.PublishCompletedEditsAsync(
                request,
                originalSubtitle.Path,
                translatedSubtitles,
                cancellationToken);
            if (!publication.Completed || string.IsNullOrWhiteSpace(publication.OutputPath))
            {
                return StatusCode(500, new
                {
                    message = publication.SkippedReason ?? "Failed to publish saved translation edits."
                });
            }

            translatedSubtitlePath = publication.OutputPath;
        ...
        {
            if (!string.IsNullOrWhiteSpace(failedSaveOwnershipToken))
            {
                await ReleaseFailedCompareSaveLeaseAsync(
                    requestId,
                    failedSaveOwnershipToken,
    ...
    private static bool TryBuildCanonicalEdits(
        IReadOnlyCollection<TranslationCompareEdit> requestedEdits,
        IReadOnlyList<SubtitleItem> originalSubtitles,
        string sourcePath,
        out Dictionary<int, string> canonicalEdits,
        out string? conflictMessage)
    {
        canonicalEdits = new Dictionary<int, string>();
        conflictMessage = null;

        var currentPositions = originalSubtitles
            .Select(subtitle => subtitle.Position)
            .ToHashSet();
        var editsByPosition = new Dictionary<int, string>();

        foreach (var edit in requestedEdits)
        {
            if (!currentPositions.Contains(edit.Position))
            {
                continue;
            }

            if (editsByPosition.TryGetValue(edit.Position, out var existingEdit))
            {
                if (!AreEquivalentEditValues(existingEdit, edit.TranslatedText))
                {
                    conflictMessage =
                        $"Conflicting edits were supplied for subtitle position {edit.Position}.";
                    return false;
                }
            ...
            editsByPosition[edit.Position] = edit.TranslatedText;
        }
        ...
        var deduplication = ProviderTextDeduper.Deduplicate(
            plan.Nodes
                ...
                .Select(node => new ProviderTextItem(
                    ...
                    node.SemanticKind))
                .ToList());
        var editsByRepresentative = new Dictionary<int, List<KeyValuePair<int, string>>>();

        foreach (var edit in editsByPosition)
        {
            if (!deduplication.RepresentativePositionByPosition.TryGetValue(
                    edit.Key,
                    out var representativePosition))
            {
                canonicalEdits[edit.Key] = edit.Value;
                continue;
            }

            if (!editsByRepresentative.TryGetValue(representativePosition, out var groupEdits))
            {
                groupEdits = [];
                editsByRepresentative[representativePosition] = groupEdits;
            }

            groupEdits.Add(edit);
        }

        foreach (var (representativePosition, groupEdits) in editsByRepresentative)
        {
            var distinctValues = groupEdits
                .Select(edit => ProviderTextDeduper.Normalize(edit.Value))
                .Distinct(StringComparer.Ordinal)
                .ToList();
            if (distinctValues.Count > 1)
            {
                var positions = deduplication
                    .GetMemberPositions(representativePosition)
                    .Order()
                    .ToList();
                conflictMessage =
                    $"Conflicting edits were supplied for semantic duplicate subtitle positions " +
                    $"{string.Join(", ", positions)}.";
                return false;
            }

            var editValue = groupEdits
                .OrderBy(edit => edit.Key)
                .First()
                .Value;
            foreach (var memberPosition in deduplication.GetMemberPositions(representativePosition))
            {
                canonicalEdits[memberPosition] = editValue;
            }
        }
    ...
    private static bool AreEquivalentEditValues(string left, string right)
    {
        return string.Equals(
            ProviderTextDeduper.Normalize(left),
            ProviderTextDeduper.Normalize(right),
            StringComparison.Ordinal);
    }

    private static async Task<string> ComputeCurrentSourceFingerprintAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return TranslationCheckpointService.GetFallbackCheckpointFingerprint(request);
        }
    }

    private List<int> ParseMissingPositions(int requestId)
    {
        var failedCues = _dbContext.TranslationFailedCues
            .Where(c => c.TranslationRequestId == requestId)
            .ToList();
        ...
        var latestReportedPositions = ParseReportedMissingPositions(latestErrorLog?.Details);
        if (latestReportedPositions.Count > 0)
        {
            var failedCuesByPosition = failedCues.ToDictionary(cue => cue.Position);
            return latestReportedPositions
                .Where(position => !failedCuesByPosition.TryGetValue(position, out var cue) ||
                                   cue.AutoApprovedAt == null)
                .Order()
                .ToList();
        }
        ...
        var positionRangeMatch = Regex.Match(
            latestErrorLog?.Details ?? string.Empty,
        ...
        positions.UnionWith(failedCues
            .Where(cue => cue.AutoApprovedAt == null)
            .Select(cue => cue.Position));

        var posMatches = Regex.Matches(latestErrorLog?.Details ?? string.Empty, @"pos (\d+):");
        ...
        return positions
            .Where(position => !failedCues.Any(cue => cue.Position == position && cue.AutoApprovedAt != null))
            .Order()
            .ToList();
    }

    private static List<int> ParseReportedMissingPositions(string? details)
    {
        ...
        var reportedPositions = new HashSet<int>();
        var positionRangeMatch = Regex.Match(
            details,
            @"missing at positions:\s*([\d,\s]+)");
        if (positionRangeMatch.Success)
        {
            foreach (var part in positionRangeMatch.Groups[1].Value.Split(
                         ',',
                         StringSplitOptions.RemoveEmptyEntries))
            {
                if (int.TryParse(part.Trim(), out var position))
                {
        ...
        foreach (Match match in Regex.Matches(details, @"pos (\d+):"))
        {
        ...
        return reportedPositions.Order().ToList();
    }
        ...
        IReadOnlyCollection<int> missingPositions)
    {
        ...
        var requiredOutputFormats = SubtitleOutputModeHelper.DeserializeFormats(request.RequiredOutputFormats);
        ...
        var preserveAssFormatting = SubtitleOutputModeHelper.IsAssFormat(sourceFormat) &&
            requiredOutputFormats.Any(SubtitleOutputModeHelper.IsAssFormat);
        var stripSubtitleFormatting = !preserveAssFormatting;
        return originalSubtitles.Select(original =>
        {
            ...
            if (hydration.Translations.TryGetValue(original.Position, out var translated))
            {
                item.TranslatedLines = FailedTranslationCheckpointRules.ReconstructCachedTranslation(
                    original,
                    translated,
                    stripSubtitleFormatting,
                    preserveAssFormatting);
            }
            else if (hydration.SourceTextPositions.Contains(original.Position))
            {
    ...
    private void CleanupTemporaryTranslatedSubtitle()
    {
    ...
    private async Task ReleaseFailedCompareSaveLeaseAsync(
        int requestId,
        ...
        List<SubtitleItem> translatedSubtitles)
    {
        var visibleOriginalSubtitles = originalSubtitles
            ...
            .ToList();
        var visibleTranslatedSubtitles = translatedSubtitles
            .Where(IsVisibleSubtitle)
            .ToList();

        var timingComparison = BuildTimingComparison(visibleOriginalSubtitles, visibleTranslatedSubtitles);
        ...
        translatedSubtitles = NormalizeTranslatedPositionsForComparison(
            originalSubtitles,
            visibleTranslatedSubtitles);
    ...
    private static string CombineSubtitleLines(SubtitleItem subtitle)
    {
        ...
        {
            return string.Join(" ", subtitle.TranslatedLines);
        }
